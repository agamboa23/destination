...
exports.trips_get_all_upcoming = (req, res, next) => {
  ...
  Trip.find({ date_of_trip: { $gt: now }, isCancelled: false })
    .select(
      "user _id origin destination date_of_trip members number_of_members"
    )
    .populate("user", "user _id first_name")
    .exec()
    .then(docs => {
...
exports.trips_get_upcoming_trips_of_user = (req, res, next) => {
  ...
  Trip.find({ date_of_trip: { $gt: now }, user: userId, isCancelled: false })
    .select(
      "user _id origin destination date_of_trip members requests number_of_members"
    )
...
exports.trips_get_completed_trips_of_user = (req, res, next) => {
  const now = new Date();
  const userId = req.params.userId;
  Trip.find({ date_of_trip: { $lt: now }, user: userId, isCancelled: false })
    .select(
      "user _id origin destination date_of_trip members requests number_of_members"
    )
    .populate("user", "user _id first_name")
...
exports.trips_get_joined_upcoming_trips_of_user = (req, res, next) => {
  let trips;
  const now = new Date();
  const userId = req.params.userId;
...
exports.trips_get_joined_completed_trips_of_user = (req, res, next) => {
  let trips;
  const now = new Date();
  const userId = req.params.userId;
  User.findById(userId).then(docs => {
...
exports.trips_get_trip = (req, res, next) => {
  Trip.findById(req.params.tripId)
    .populate("user")
    .select(
      "user _id origin destination date_of_trip date_of_publish description members number_of_members requests isCancelled"
    )
    .exec()
    .then(doc => {
      if (!doc) {
...
async function makeAcceptNotification(tripId, userId) {
  ...
  const notification = new Notification({
    ...
    isRead: false,
    message:
      "Your request to join the trip to " + destination + " was accepted."
  });
...
exports.trip_reject_request = (req, res, next) => {
  const tripId = req.params.tripId;
  const userId = req.params.userId;
  Trip.update({ _id: tripId }, { $pull: { requests: { $in: [userId] } } })
    .exec()
    .then(result => {
      res.status(200).json({
        message: "Trip updated",
...
async function makeRejectionNotification(tripId, userId) {
  ...
  const notification = new Notification({
    ...
    isRead: false,
    message:
      "Your request to join the trip to " + destination + " was rejected."
  });
  return notification
    .save()
    .then(doc =>
