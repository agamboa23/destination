...
export async function get_districts_recommendations(req, res, next) {
  ...
  const pProvinceId =
    req.params.province_osm_id && isNaN(req.params.province_osm_id)
      ? -1
      : req.params.province_osm_id;
  const findQuery = req.params.province_osm_id
    ? { parent_osm_id: pProvinceId }
    : {};
  var wikidata,
    districts_qids = "";
  try {
    const districts = await District.find(findQuery)
      .skip(qSkip)
      .limit(qLimit)
      .lean()
      .sort([[qSortBy, qSortCriteria]])
    ...
    if (includeImage) {
      districts.forEach(
        district => (districts_qids += district.wikidata_code + "|")
      );
...
export async function get_dstn_recommendations(req, res, next) {
  ...
  if (isNaN(pDistrictId)) {
    return res
      .status(400)
      .json({ message: "Bad Request, invalid district id" });
  }
...
export async function get_dstn_by_stereotype(req, res, next) {
  ...
  const qMaxDistance = isNaN(req.query.maxDistance)
    ? 15
    : Number(req.query.maxDistance);
  const qMinDistance = isNaN(req.query.minDistance)
    ? 0
  ...
  if (qLocation) {
    ...
    if (location.length < 2 || isNaN(location[0], isNaN(location[1]))) {
      return res.status(400).json({
        message: "Bad Request, invalid location latitude and longitude"
      });
    }
    ...
    if (qMinDistance == 0) {
      queryString = opQueryBuilder.get_around_query(
        location.toString(),
        filters,
        maxDistance,
        pBtReachable
      ...
      queryString = opQueryBuilder.get_ring_query(
        location.toString(),
        filters,
        maxDistance,
        minDistance,
    ...
    if (isNaN(qArea)) {
      return res
        .status(400)
        .json({ message: "Bad Request, invalid area code" });
    }
    overpassAreaCode = areaPrefix + Number(qArea);
    overpassAreaCode = "area:" + overpassAreaCode;
    queryString = opQueryBuilder.get_area_query(
      overpassAreaCode,
      filters,
      pBtReachable
    );
...
export async function sort_rank(req, res, next) {
  ...
  var fb_id = req.body.user_id;
  try {
    var ranked_list = await RecRanker.rank_destinations(req.body.destination_list,rank_type,fb_id);
    if (ranked_list.error) {
      res.status(200).json({ destinations:ranked_list.destinations,error:"Not Available"});
    ...
    res.status(200).json({ destinations:req.body.destination_list,error:error});
    console.log(error);
  }
}